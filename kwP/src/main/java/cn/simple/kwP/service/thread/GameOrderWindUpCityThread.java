 ...
 * 自动结算红包运动赛
 */
public class GameOrderWindUpCityThread implements Runnable {

	Logger logger = Logger.getLogger(GameOrderWindUpCityThread.class);
	...
	public void run() {
		try {
			...
			String cityG = "group_game_order_city";
			String no = "";
			...
			for (int i = 1; i <= 64; i++) {
				no = "_" + i;
				Db.update(update_gtb.replace("XXX", cityG + no), beforePhaseNo);

				// 获取需要结算的信息
				List<GroupGameOrderCity> dataList = GroupGameOrderCity.dao.find(select_gtb.replace("XXX", cityG + no),
						beforePhaseNo);
				if (dataList != null) {
					Integer cityId = null;
					String groupNo = "";
					...
					for (GroupGameOrderCity info : dataList) {
						cityId = info.getCityId();
						groupNo = info.getGroupNo();
						key = cityId + "_" + groupNo;
						userId = info.getUserId();
						amount = info.getAmount();
						tbName = ConsistentHash.getTbName(userId.toString(), tb);
						insterParams = "(" + userId + ",'" + beforePhaseNo + "','1','" + info.getCityName() + "','1',"
			...
			for (JSONObject params : userAccountMap.values()) {
				...
				update_account = update_account.append("update user_account set usable_balance=usable_balance + ?, "
						+ "income_city_count=income_city_count + 1, "
						+ "income_city_amount=income_city_amount + ? where user_id in ");
			...
			e.printStackTrace();
			logger.error("--------城市赛结算线程处理异常--------" + e.toString());
		}
	}
}